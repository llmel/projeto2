 ...
 *
 * @author dblock[at]dblock[dot]orN
 */
...
// MEMORIA RAM --- TRANSFORMAR EM %%%%
    private static  void printMemory(GlobalMemory memory) {
        ...
        System.out.println("MEMORIA: " + memoriaPorcent + "%");
    }
...
// USER É A %%%% DA CPU
        System.out.format(
                "CPU: %.1f%% ",
                  //      + "Nice: %.1f%% System: %.1f%% Idle: %.1f%% IOwait: %.1f%% IRQ: %.1f%% SoftIRQ: %.1f%%%n",
                100d * user / totalCpu);
...
// DISCO --- TRANSFORMAR RESTO EM PORCENTAGEM
        OSFileStore[] fsArray = fileSystem.getFileStores();
        for (OSFileStore fs : fsArray) {
            long usable = fs.getUsableSpace();
            long total = fs.getTotalSpace();
            
            long disco_utilizado = total - usable;
            long discoPorcent = disco_utilizado * 100 / total;
            
             System.out.println("DISCO: " + discoPorcent + "%");
